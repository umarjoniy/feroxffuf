...
set edit:completion:arg-completer[feroxbuster] = {|@words|
    ...
    var completions = [
        &'feroxbuster'= {
            ...
            cand -w 'Wordlist file. Append :KEYWORD to set a custom fuzzing placeholder. Default keyword is FUZZ. Repeatable for multi-keyword modes: -w list.txt               (keyword = FUZZ) -w users.txt:USER         (keyword = USER) -w users.txt:USER -w pass.txt:PASS  (pitchfork / cluster-bomb)'
            cand --wordlist 'Wordlist file. Append :KEYWORD to set a custom fuzzing placeholder. Default keyword is FUZZ. Repeatable for multi-keyword modes: -w list.txt               (keyword = FUZZ) -w users.txt:USER         (keyword = USER) -w users.txt:USER -w pass.txt:PASS  (pitchfork / cluster-bomb)'
            cand --mode 'Fuzzing attack mode (active when FUZZ keyword is present): clusterbomb  cartesian product of all wordlists (default) pitchfork    parallel zip, stops at shortest list sniper       one FUZZ position at a time, single wordlist'
            cand --fuzz-recurse-depth 'Maximum recursion depth in fuzz mode (default: 4)'
            cand --fuzz-recurse-status 'Comma-separated status codes that trigger recursion Default: 200,301,302,307,308'
            cand --fuzz-recurse-match 'Regex matched against response URL to trigger recursion e.g. --fuzz-recurse-match ''/dir/$'' to recurse only on trailing-slash responses'
            ...
            cand --fuzz-recurse 'Enable recursion in fuzz mode (off by default). Path mode: found dirs queued as <dir>/<KEYWORD>. VHost mode (with --fuzz-recurse-vhost): found host queued as <KEYWORD>.<host>.'
            cand --fuzz-recurse-vhost 'When fuzzing Host header, recurse into discovered subdomains: e.g. Host: FUZZ.example.com finds api.example.com next round: Host: FUZZ.api.example.com'
