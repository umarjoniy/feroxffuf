...
Register-ArgumentCompleter -Native -CommandName 'feroxbuster' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'feroxbuster' {
            ...
            [CompletionResult]::new('-w', '-w', [CompletionResultType]::ParameterName, 'Wordlist file. Append :KEYWORD to set a custom fuzzing placeholder. Default keyword is FUZZ. Repeatable for multi-keyword modes: -w list.txt               (keyword = FUZZ) -w users.txt:USER         (keyword = USER) -w users.txt:USER -w pass.txt:PASS  (pitchfork / cluster-bomb)')
            [CompletionResult]::new('--wordlist', '--wordlist', [CompletionResultType]::ParameterName, 'Wordlist file. Append :KEYWORD to set a custom fuzzing placeholder. Default keyword is FUZZ. Repeatable for multi-keyword modes: -w list.txt               (keyword = FUZZ) -w users.txt:USER         (keyword = USER) -w users.txt:USER -w pass.txt:PASS  (pitchfork / cluster-bomb)')
            [CompletionResult]::new('--mode', '--mode', [CompletionResultType]::ParameterName, 'Fuzzing attack mode (active when FUZZ keyword is present): clusterbomb  cartesian product of all wordlists (default) pitchfork    parallel zip, stops at shortest list sniper       one FUZZ position at a time, single wordlist')
            [CompletionResult]::new('--fuzz-recurse-depth', '--fuzz-recurse-depth', [CompletionResultType]::ParameterName, 'Maximum recursion depth in fuzz mode (default: 4)')
            [CompletionResult]::new('--fuzz-recurse-status', '--fuzz-recurse-status', [CompletionResultType]::ParameterName, 'Comma-separated status codes that trigger recursion Default: 200,301,302,307,308')
            [CompletionResult]::new('--fuzz-recurse-match', '--fuzz-recurse-match', [CompletionResultType]::ParameterName, 'Regex matched against response URL to trigger recursion e.g. --fuzz-recurse-match ''/dir/$'' to recurse only on trailing-slash responses')
            ...
            [CompletionResult]::new('--fuzz-recurse', '--fuzz-recurse', [CompletionResultType]::ParameterName, 'Enable recursion in fuzz mode (off by default). Path mode: found dirs queued as <dir>/<KEYWORD>. VHost mode (with --fuzz-recurse-vhost): found host queued as <KEYWORD>.<host>.')
            [CompletionResult]::new('--fuzz-recurse-vhost', '--fuzz-recurse-vhost', [CompletionResultType]::ParameterName, 'When fuzzing Host header, recurse into discovered subdomains: e.g. Host: FUZZ.example.com finds api.example.com next round: Host: FUZZ.api.example.com')
